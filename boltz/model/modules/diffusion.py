...
class AtomDiffusion(Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        solver : str, optional
            The solver to use for sampling. Options are "euler" (default),
            "dpm_1" (DPM-Solver-1, first-order), "dpm_2" (DPM-Solver-2, second-order).
        ...
        # Solver selection
        valid_solvers = ["euler", "dpm_1", "dpm_2"]
    ...
    def _euler_step(
        ...
        """Euler method step (default solver)."""
        denoised_over_sigma = (x_noisy - x_denoised) / sigma_curr
        return x_noisy + self.step_scale * (sigma_next - sigma_curr) * denoised_over_sigma

    def _dpm_solver_1_step(
        self,
        x_noisy: torch.Tensor,
        x_denoised: torch.Tensor,
        sigma_curr: float,
        ...
        """DPM-Solver-1 step (first-order, similar to DDIM).

        For EDM parameterization where the model predicts x_0:
        x_{t-1} = x_0 + (sigma_{t-1} / sigma_t) * (x_t - x_0)
        """
        if sigma_next == 0:
            return x_denoised
        ratio = sigma_next / sigma_curr
        return x_denoised + ratio * (x_noisy - x_denoised)

    def _dpm_solver_2_step(
        self,
        x_noisy: torch.Tensor,
        x_denoised: torch.Tensor,
        x_denoised_prev: torch.Tensor,
        sigma_curr: float,
        sigma_next: float,
        sigma_prev: float,
    ) -> torch.Tensor:
        """DPM-Solver-2 step (second-order).

        Uses linear extrapolation of denoised predictions for higher accuracy.
        Falls back to first-order if no previous estimate is available.
        """
        if sigma_next == 0:
            return x_denoised

        if x_denoised_prev is None or sigma_prev is None:
            # Fall back to first-order
            return self._dpm_solver_1_step(x_noisy, x_denoised, sigma_curr, sigma_next)

        # Compute log-sigma ratios for second-order update
        # Using the EDM parameterization with log-SNR space interpolation
        log_sigma_curr = log(sigma_curr)
        log_sigma_next = log(sigma_next)
        log_sigma_prev = log(sigma_prev)

        h = log_sigma_next - log_sigma_curr
        h_prev = log_sigma_curr - log_sigma_prev

        # Second-order coefficient
        r = h_prev / h

        # Linear extrapolation of denoised predictions
        D_curr = (x_noisy - x_denoised) / sigma_curr
        D_prev = (x_noisy - x_denoised_prev) / sigma_prev if sigma_prev > 0 else D_curr

        # DPM-Solver-2 update
        ratio = sigma_next / sigma_curr
        x_next = x_denoised + ratio * (x_noisy - x_denoised)

        # Second-order correction
        coef = (1 / (2 * r)) * (sigma_next - sigma_curr)
        x_next = x_next + coef * (D_curr - D_prev)
    ...
    def sample(
        ...
        **network_condition_kwargs,
        ...
        # For DPM-Solver-2: track previous denoised estimate and sigma
        atom_coords_denoised_prev = None
        sigma_prev = None
        ...
        for step_idx, (sigma_tm, sigma_t, gamma) in enumerate(sigmas_and_gammas):
            ...
            # Apply solver-specific update step
            ...
            elif self.solver == "dpm_1":
                atom_coords_next = self._dpm_solver_1_step(
                    atom_coords_noisy, atom_coords_denoised, t_hat, sigma_t
                )
            elif self.solver == "dpm_2":
                atom_coords_next = self._dpm_solver_2_step(
                    atom_coords_noisy,
                    atom_coords_denoised,
                    atom_coords_denoised_prev,
                    t_hat,
                    sigma_t,
                    sigma_prev,
                )
                # Update previous estimates for next iteration
                atom_coords_denoised_prev = atom_coords_denoised.clone()
                sigma_prev = t_hat
