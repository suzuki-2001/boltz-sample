...
if TYPE_CHECKING:
    from boltz.callbacks.progress import BoltzProgressCallback
...
class BoltzWriter(BasePredictionWriter):
    ...
    def __init__(
        ...
        boltz2: bool = False,
        progress_callback: Optional["BoltzProgressCallback"] = None,
    ) -> None:
        ...
        progress_callback : BoltzProgressCallback, optional
            Callback to report progress to.
    ...
    def write_on_batch_end(
        ...
        if prediction["exception"]:
            ...
            if self.progress_callback:
                self.progress_callback.mark_failed(record_id)
            return

        if self.progress_callback:
            self.progress_callback.mark_completed(record_id)
