...
def _(Path, pd, plt):
    ...
    ax.set_title("RfaH Structure Prediction", fontsize=11)
...
def _(col_x, col_y, df, mo, ref_x, ref_y):
    ...
    mo.md(f"""
    **Best prediction for {ref_x} (alpha-helix CTD):**
    - beta={best_ref1['beta']:+.2f}, {idx_col}={int(best_ref1[idx_col])}, TM-score={best_ref1[col_x]:.3f}

    **Best prediction for {ref_y} (beta-barrel CTD):**
...
def _(Path, best_ref1, best_ref2, idx_col, mo, ref_x, ref_y):
    ...
    def get_prediction_path(beta: float, model_idx: int) -> Path:
        ...
        return beta_dir / "rfah" / f"rfah_model_{model_idx}.cif"
...
def _(script_dir, show_structure):
    # Reference: 2OUG (alpha-helix CTD conformation)
    show_structure(
        script_dir / "data" / "ref" / "2OUG_C.pdb", "Reference: 2OUG (alpha-helix CTD)"
    )
    return
...
@app.cell
def _(script_dir, show_structure):
    # Reference: 6C6S (beta-barrel CTD conformation)
    show_structure(
        script_dir / "data" / "ref" / "6C6S_D.pdb", "Reference: 6C6S (beta-barrel CTD)"
    )
    return
...
def _(best_ref1, best_ref1_path, col_x, ref_x, show_structure):
    # Best prediction matching alpha-helix CTD (colored by pLDDT confidence)
    show_structure(
        best_ref1_path,
        f"Best prediction for alpha-helix CTD ({ref_x}): beta={best_ref1['beta']:+.2f}, TM={best_ref1[col_x]:.3f}",
...
def _(best_ref2, best_ref2_path, col_y, ref_y, show_structure):
    # Best prediction matching beta-barrel CTD (colored by pLDDT confidence)
    show_structure(
        best_ref2_path,
        f"Best prediction for beta-barrel CTD ({ref_y}): beta={best_ref2['beta']:+.2f}, TM={best_ref2[col_y]:.3f}",
        color_by_confidence=True,
    )
    return
